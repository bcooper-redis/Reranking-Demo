...
from app.api.context import evaluation, telemetry
...
async def record_click(request: Request, payload: ClickEventRequest) -> EventReceipt:
    event_id = await telemetry(request).record_click(payload)
...
async def telemetry_snapshot(request: Request) -> TelemetrySnapshot:
    return telemetry(request).snapshot()
...
async def run_evaluation(
    ...
    try:
        return await evaluation(request).run(payload.reranker_id if payload else "minilm_l6")
...
async def run_load_test(request: Request, payload: LoadTestRequest) -> LoadTestRun:
    try:
        return await evaluation(request).run_load(
...
async def get_evaluation(request: Request, evaluation_id: str) -> EvaluationRun:
    result = await evaluation(request).get(evaluation_id)
    if result is None:
        raise HTTPException(
